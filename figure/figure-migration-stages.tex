...
\coordinate (dockerHubco) at (8.5,7);
...
% 
% Server OS Node A
\node [name=srvosA, srvos,draw] at (serverAco) {OS(Linux)};
% Docker Node
...
% Server OS Node B
\node [name=srvosB, srvos,draw] at (serverBco) {OS(Linux)};
% Docker Node
...
% % sloped, 
% anchor=center, above] {
% % \ref{predownload}. Download Images
% }
(aufsB.north);
...
\ref{predownload}. Pre-Download Images
}
...
node[right, black]{
\ref{prepare}. Migration Request
}
([xshift=2em]srvosA.south);
...
node[above, black]{
\ref{prepare}. Match Image Layers
}
node[below, black]{
...
\draw[<->] 
([xshift=2.8em]dockerA.north) 
-- 
node[black, right,text width=2cm,align=left
% , text width=2cm
]{
\ref{prepare}. ID Remapping
} 
([xshift=0em]aufsA.south);
